...
#define LEN_ICM_BUFFER        23

static uint8_t icm_buffer[LEN_ICM_BUFFER];  // persistent in memory
...
void ICM20948_Update_Raw_Data(void)
{
	ICM20948_gyro_read(&g_raw_gyro);
	ICM20948_accel_read(&g_raw_accel);
...
void ICM20948_Send_Raw_Data(void)
{
	...
	icm_buffer[0] = PTO_HEAD;
	icm_buffer[1] = PTO_DEVICE_ID - 1;
	icm_buffer[2] = LEN_ICM_BUFFER - 2;
	icm_buffer[3] = FUNC_REPORT_MPU_RAW;
	icm_buffer[4] = g_raw_gyro.x & 0xff;
	icm_buffer[5] = (g_raw_gyro.x >> 8) & 0xff;
	icm_buffer[6] = g_raw_gyro.y & 0xff;
	icm_buffer[7] = (g_raw_gyro.y >> 8) & 0xff;
	icm_buffer[8] = g_raw_gyro.z & 0xff;
	icm_buffer[9] = (g_raw_gyro.x >> 8) & 0xff;

	icm_buffer[10] = g_raw_accel.x & 0xff;
	icm_buffer[11] = (g_raw_accel.x >> 8) & 0xff;
	icm_buffer[12] = g_raw_accel.y & 0xff;
	icm_buffer[13] = (g_raw_accel.y >> 8) & 0xff;
	icm_buffer[14] = g_raw_accel.z & 0xff;
	icm_buffer[15] = (g_raw_accel.z >> 8) & 0xff;

	icm_buffer[16] = g_raw_mag.x & 0xff;
	icm_buffer[17] = (g_raw_mag.x >> 8) & 0xff;
	icm_buffer[18] = g_raw_mag.y & 0xff;
	icm_buffer[19] = (g_raw_mag.y >> 8) & 0xff;
	icm_buffer[20] = g_raw_mag.z & 0xff;
	icm_buffer[21] = (g_raw_mag.z >> 8) & 0xff;

	for (i = 2; i < LEN_ICM_BUFFER - 1; i++)
	{
		checknum += icm_buffer[i];
	}
	icm_buffer[LEN_ICM_BUFFER - 1] = checknum;

	USART1_Send_ArrayU8(icm_buffer, sizeof(icm_buffer));
